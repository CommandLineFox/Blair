...
import {blockFreshInteraction, fetchChannelFromGuild, fetchMember, isStaff} from '../../utils/utils';
...
export class KickButtonHandler extends InteractionHandler {
    ...
    public async run(interaction: ButtonInteraction): Promise<void> {
        ...
        const channel = await fetchChannelFromGuild(interaction.guild, interaction.channelId);
        if (!channel) {
        ...
        if (!userPermissions?.has(PermissionFlagsBits.KickMembers)) {
            await interaction.editReply({ content: "You don't have the kick members permission." });
            return;
        }

        const botPermissions = interaction.guild.members.me?.permissions;
        if (!botPermissions?.has(PermissionFlagsBits.KickMembers)) {
            await interaction.editReply({ content: "The bot doesn't have the kick members permission in that channel" });
            return;
        }
